...
DECLARE
  ...
  v_wave7_wave9_full_closure_objects NUMBER;
BEGIN
  SELECT COUNT(*)
    INTO v_table_count
    FROM USER_TABLES
  ...
  SELECT COUNT(*)
   ...
   WHERE SEQUENCE_NAME IN (
     ...
     'SEQ_WORKBENCH_SAVED_FILTER',
     'SEQ_WORKBENCH_EXPORT_BATCH',
     'SEQ_BULK_OPERATION_BATCH',
     'SEQ_BULK_OPERATION_ROW',
     'SEQ_ASSIGNMENT_PROPOSAL_CONTEXT',
     'SEQ_STORED_FILE',
     'SEQ_PROCEEDINGS_EXPORT_FILE',
     'SEQ_DOI_INDEX_ADAPTER_SUB',
     'SEQ_COMMUNICATION_COMPOSE_BATCH',
     'SEQ_COMMUNICATION_REMINDER'
   );
  ...
  SELECT COUNT(*)
   ...
   WHERE TRIGGER_NAME IN (
     ...
     'TRG_WORKBENCH_SAVED_FILTER_BIU',
     'TRG_WORKBENCH_EXPORT_BATCH_BI',
     'TRG_BULK_OPERATION_BATCH_BI',
     'TRG_BULK_OPERATION_ROW_BI',
     'TRG_ASSIGNMENT_PROPOSAL_CTX_BI',
     'TRG_STORED_FILE_BI',
     'TRG_PROCEEDINGS_EXPORT_FILE_BI',
     'TRG_DOI_INDEX_ADAPTER_SUB_BI',
     'TRG_COMM_COMPOSE_BATCH_BI',
     'TRG_COMMUNICATION_REMINDER_BI'
   );
  ...
  SELECT COUNT(*)
    INTO v_wave7_wave9_full_closure_objects
    FROM (
      SELECT TABLE_NAME AS OBJECT_NAME
      FROM USER_TABLES
      WHERE TABLE_NAME IN (
  ...
  IF v_table_count <> 68 THEN
    RAISE_APPLICATION_ERROR(-20001, 'Expected 68 tables, found ' || v_table_count);
  END IF;

  IF v_sequence_count <> 67 THEN
    RAISE_APPLICATION_ERROR(-20002, 'Expected 67 sequences, found ' || v_sequence_count);
  END IF;

  IF v_trigger_count <> 70 THEN
    RAISE_APPLICATION_ERROR(-20003, 'Expected 70 triggers, found ' || v_trigger_count);
  END IF;
  ...
  IF v_wave7_wave9_full_closure_objects <> 40 THEN
    RAISE_APPLICATION_ERROR(-20025, 'Wave7/Wave9 full closure schema objects are missing');
  END IF;
